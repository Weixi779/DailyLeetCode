...
## 运行
```bash
swift run DailyLeetCodeRunner            # 运行全部任务（若有）
swift run DailyLeetCodeRunner LT0088     # 按题号运行
swift run DailyLeetCodeRunner --tag=daily  # 按标签运行
```
快捷方式：在仓库根目录使用 `scripts/dl ...`，无需输入完整 `swift run`。
...
```
自动抓取中文题面/难度，生成文件并注册到 `ProblemCatalog`。支持 `--force` 覆盖，`--env=<path>` 指定凭证。
